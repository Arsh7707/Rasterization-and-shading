...
double dotGridGradient(int ix, int iy, double x, double y)
{
    ...
    // Retrieve the gradient vector from the grid
    Eigen::Vector2d gradient = grid[iy][ix];
...
double ray_sphere_intersection(const Vector3d &ray_origin, const Vector3d &ray_direction, int index, Vector3d &p, Vector3d &N)
{
    ...
    double t_ca = L.dot(ray_direction);
    ...
    // Check if both t0 and t1 are negative, if so the sphere is behind the ray
    if (t0 < 0 && t1 < 0) return -1;

    // The smallest positive t is the closest intersection point
    double t = t0 < 0 ? t1 : t0;
...
double ray_parallelogram_intersection(const Vector3d &ray_origin, const Vector3d &ray_direction, int index, Vector3d &p, Vector3d &N)
{
    ...
    // Compute the normal to the parallelogram
    N = pgram_u.cross(pgram_v).normalized();

    // Calculate the intersection point with the plane defined by the parallelogram
    double denom = N.dot(ray_direction);
    if (fabs(denom) > std::numeric_limits<double>::epsilon()) {
        Vector3d pgram_origin_to_ray_origin = ray_origin - pgram_origin;
        double t = -pgram_origin_to_ray_origin.dot(N) / denom;

        // Check if the intersection point lies within the parallelogram
        Vector3d intersection_point = ray_origin + t * ray_direction;
        Vector3d AP = intersection_point - pgram_origin;
        double u = AP.dot(pgram_u) / pgram_u.squaredNorm();
        double v = AP.dot(pgram_v) / pgram_v.squaredNorm();

        if (u >= 0 && u <= 1 && v >= 0 && v <= 1) {
            // Intersection point lies within the parallelogram
...
{
     Vector3d p, N;
    int nearest_object_index = find_nearest_object(ray_origin, ray_direction, p, N);

    if (nearest_object_index < 0) {
        // No object is intersected, so the light is visible
        return true;
    }
    ...
    // Compute the distance from the ray origin to the light source
    double distance_to_light = (light_position - ray_origin).norm();

    // If the intersection is farther than the light source, the light is visible
    if (distance_to_intersection > distance_to_light) {
        return true;
    }

    // Otherwise, the light is occluded by an object
    return false;
}
...
Vector4d shoot_ray(const Vector3d &ray_origin, const Vector3d &ray_direction, int max_bounce)
{
    ...
    for (int i = 0; i < light_positions.size(); ++i)
    {
        ...
        const Vector4d specular = obj_specular_color * (pow(std::max((N.dot(h)), 0.), obj_specular_exponent));
...
void raytrace_scene()
{
    ...
    // Reduce the focal length to make the image appear closer
    ...
    Vector3d camera_right = camera_direction.cross(world_up).normalized();

    // Compute camera_up as the cross product of camera_right and camera_direction
    Vector3d camera_up = camera_right.cross(camera_direction).normalized();
